...
via :class:`~palm.system.supervisor.SystemSupervisor` when the host wires them.
"""
...
class InboundBindingService:
    ...
    def status(self) -> dict[str, Any]:
        """Doctor-oriented snapshot for the supervisor."""
        with self._lock:
            bindings = {
                name: {
                    "mode": b.spec.mode,
                    "status": b.status,
                    "work": b.spec.work.target if b.spec.work else None,
                }
                for name, b in self._bindings.items()
            }
            workers = sum(1 for t in self._worker_threads.values() if t.is_alive())
        return {
            "name": "inbound",
