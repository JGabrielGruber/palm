...
| [SD-020](#sd-020) | Dual readiness / residual edges (no single admission gate) | S1 | L | **0.63** floor | open (0.66 face; 0.67.1 require; 0.67.2 work-plane able; host start_ports [VISION-0.67](docs/vision/VISION-0.67.md)) |
...
**Status:** open (0.63 walls up; residuals **named**). **0.66** paid the face (installed names on the snapshot). **0.67.1** paid the organ door (`require_capability`). **0.67.2** paid work-plane `able` (drain membership; wait stays ready). Host `start_ports.able` remains `_started` until José includes it. Sequence: [VISION-0.64](docs/vision/closed/VISION-0.64.md).
