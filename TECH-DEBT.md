...
| **0.53** | **Sovereign Runners** — NeonRoot as provider | 🟡 open (0.53.0) · [VISION-0.53](docs/VISION-0.53.md) · [ADR-022](docs/adr/022-neonroot-provider.md) | `health`/`spawn`; tool images; optional install; enables thin workspace |
| **0.54** | **Library Pipeline** — Palm builds the canopy | queued · [VISION-0.54](docs/VISION-0.54.md) | Palm pipeline/wizard on neonroot resources — not `subprocess(just)` dogfood |
