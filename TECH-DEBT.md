...
| [SD-020](#sd-020) | Dual readiness / residual edges (no single admission gate) | S1 | L | **0.63** floor | open (0.66 face; 0.67.1 require door; able/façades [VISION-0.67](docs/vision/VISION-0.67.md)) |
...
**Status:** open (0.63 walls up; residuals **named**). **0.66** paid the face (installed names on the snapshot). **0.67.1** paid the organ door (`require_capability`). Remaining dependents (`able`, façades) are [VISION-0.67](docs/vision/VISION-0.67.md). Sequence: [VISION-0.64](docs/vision/closed/VISION-0.64.md).
