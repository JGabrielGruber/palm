...
**Assembly:** authority issues definition; pure engine reconciles; system applies effect intents; **admission** gates business that needs ground and publishes installed capabilities; an act that needs an organ uses `require_capability`; work-plane `able` (kernel and host start ports) is drain membership, wait stays ready; clients use ports; composition root only wires. New definition → reassemble. Theme: [VISION-0.67](vision/VISION-0.67.md) (**open**) · prior [VISION-0.66](vision/closed/VISION-0.66.md) (**closed**) · law: [VISION-ASSEMBLY](vision/VISION-ASSEMBLY.md).
