...
**Status:** 📗 **Theme open** (José 2026-08-20). Execute `0.67.2` landed. Package stamp stays `0.66.0` until exit.  
...
Façades and tests that freeze `may_run_business` as the organ list. Work-plane `able` that should mean drain is here. Host `start_ports.able` (today `_started` only) if José puts it in this minor.
...
| **0.67.3+** | Host `start_ports.able` if José includes it. Leftover façades. |
