...
| **B3** | **Landed.** Presets and seed fold do not write `"work_drain"`. Embedded definition no longer refuses `background_drain`. Omit is enough. Journal composition king paid 0.67.7. Projections composition king paid 0.67.9. | Do not wipe `composition.has` for webhook / compensation / analytics without the 0.64 shape. |
| **C** | **Landed.** Coordinator and host read `runtime.work_plane`. `host.start_plane` / `_start_plane` gone. Host schedule ends at ready. Drain start is `system.background.start`. Assemble uses `shell.structure`. Settings/deployment have no `enable_work_drain_service`. Vitality default probes share `attr_resolver` / hub / `first_resolver`. Journal attach hand landed 0.67.7; leftover costume paid 0.67.8 (one organ on the runtime bus). Projections leftover costume paid 0.67.10 (one organ on the runtime bus). Compensation DNA + attach hand landed 0.67.11 (dual-attach leftover). | No definition `requires`. |
