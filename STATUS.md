...
**Last Updated:** August 21, 2026 · 0.67.12 landed (compensation leftover: one organ). Package stays `0.66.0` until exit.  
...
**Debt (live):** [TECH-DEBT.md](TECH-DEBT.md) — **SD-020** (face paid; require door landed; work-plane able paid; host start_ports paid; capability voice paid; schedule fire paid; vitality work_cycle paid; journal king paid; journal leftover paid; projections king paid; projections leftover paid; compensation king paid; compensation leftover paid) · **SD-021** · **SD-022** · residual **SD-019** · **SD-016** / **BI-*** / **SI-*** / **SU-***  
...
José opened 0.67 (2026-08-20). **0.67.12** landed: compensation leftover is one organ on the runtime bus; recovery slot aliases it. Ready-only `require_business_admission` sites stay. Exit is José.
...
| **Later** | [SD-021](TECH-DEBT.md#sd-021) remaining composition kings (webhook / analytics) · [VISION-0.56](docs/vision/VISION-0.56.md) · [NAVIGATOR](docs/vision/VISION-NAVIGATOR.md) · [TUNNELS](docs/vision/VISION-TUNNELS.md) → [Grove](docs/vision/VISION-GROVE.md) |
| **Experimental** | Pre-1.0 · **no LTS** — [README](README.md) |

## 0.67 — Dependents require the organ (**open** · José 2026-08-20 · execute `0.67.12`)
...
An act that needs an organ requires that organ on the published gate. Ready stays the short wall.

| Patch | Status |
|-------|--------|
| **0.67.0** | ✅ plan + ADR-036 Proposed + locks |
| **0.67.1** | ✅ `require_capability` + `CapabilityRefusedError` + drain-shaped proof |
| **0.67.2** | ✅ work-plane `able` = ready then `work_drain`; wait stays ready |
| **0.67.3** | ✅ host `start_ports.able` = drain; wait stays ready |
| **0.67.4** | ✅ surface voice for `CapabilityRefusedError` (`capability_refused`) |
| **0.67.5** | ✅ `tick_schedules` / host `tick_work` use drain able |
| **0.67.6** | ✅ vitality `work_cycle` drain proofs pin `local.cli` |
| **0.67.7** | ✅ journal DNA + attach hand; `composition.has` dies on that unit |
| **0.67.8** | ✅ journal leftover: one organ on the runtime bus; not a loop |
| **0.67.9** | ✅ projections DNA + attach hand; `composition.has` dies on that unit |
| **0.67.10** | ✅ projections leftover: one organ on the runtime bus; not a loop |
| **0.67.11** | ✅ compensation DNA + attach hand; `composition.has` dies on that unit |
| **0.67.12** | ✅ compensation leftover: one organ on the runtime bus; not a loop |
| **0.67.13+** | 📋 more composition kings if José includes them |
| **exit** | ADR-036 Accepted · stamp `0.67.0` · José |

**Floor:** sibling require + one drain-shaped proof (**0.67.1**). **Not this theme:** remaining SD-021 kings unless included · engine-as-walker · definition `requires`.

## Later seeds (not this season)

| Seed | Home |
|------|------|
| Remaining `composition.has` kings | [SD-021](TECH-DEBT.md#sd-021) — webhook / analytics |
