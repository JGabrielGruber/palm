...
**0.64 is `work_drain` as the first real capability.** Walker + seats + no freelance wire + composition does not write the name. Costume field, seed twins, plane aliases, assemble getattr scrape, and the settings/deployment drain switch are gone. Vitality probes assembly with `attr_resolver("assembly")` (same as supervisor). Next: José exits when the organ is copyable. No patch queue.
