...
**Last Updated:** August 24, 2026 · 0.68.2 composts runner `ready()` doctor register. Package stays `0.67.0` until 0.68 exit.  
...
José closed 0.67 (2026-08-21) and opened 0.68 as costume compost. **0.68.2** composts runner `ready()` doctor register. Further units when José names them.
...
| **0.68.3+** | 📋 compost units José includes |
| **exit** | stamp `0.68.0` · José |

**Named remaining:** bare `enable_event_outbox` · living POST lies · twin seats that pin empty work. `enable_state_snapshot` stays read packaging.
