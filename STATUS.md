...
**0.64 is `work_drain` as the first real capability.** Walker + seats + no freelance wire + composition does not write the name. Omit is enough. Next: cheap costume (A) or coordinator alias (C). José exits when the organ is copyable. No patch queue.
